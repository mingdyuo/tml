...
- `C++`의 객체(변수) 초기화에 대해선 규칙이 명확이 존재한다. 

  근데 이걸 다 기억할만한 가치가 있기에는 너무 복잡하므로 **걍 모든 객체를 죄다 초기화 시키자.**

- build-in type은 손수 초기화 하자.

  나머지 부분은 생성자에서 초기화 하자. 생성자 본문이 시작되기 전에 멤버 초기화 리스트를 사용한다.

- 멤버변수가 user defined type인 경우에는 초기화 값을 넣어주지 않는 경우 해당 타입의 기본 생성자가 호출된다. 

- `C++`에서 객체를 구성하는 데이터의 초기화 순서는 컴파일러 상관 없이 똑같다.

  - 기본클래스 → 파생 클래스

  - 클래스 데이터 멤버는 선언된 순서대로 초기화

    가능하면 초기화 리스트의 순서와 데이터 선언 순서를 맞춰주자.

- 비지역 정적 객체의 초기화 순서는 개별 번역 단위에서 정해진다.

  > **정적 객체**는 생성 시점부터 프로그램이 끝날 때까지 살아 있는 객체다.
  >
  > 비지역 정적 객체들
  >
  > - 전역 객체
  ...
  번역 단위는 컴파일을 통해 하나의 목적 파일을 만드는 바탕이 되는 소스 코드를 의미한다. 기본적으로 [소스 파일 하나 + `#include`하는 파일] 까지 합쳐서 번역 단위가 됨

  다른 번역 단위에 있는 비지역 정적 객체는 초기화 순서가 정해져 있지 않다. 어떤 정적 객체가 초기화 되기 전에 다른 객체에 의해 호출된다면 큰일!

  → 이를 해결하는 방법 중 하나가 싱글톤 패턴이다. 함수 안에 정적 객체를 넣어서 지역 정적 객체로 바꿔 버리는 것

- 멀티 스레드 환경에서 상수가 아닌 정적 객체는 시한 폭탄이 될 수 있다 ㅎ 

- 아무튼, 초기화 순서가 불확실하다는 것을 염두하고, 이런 불확실성을 피해서 프로그램을 설계해라
...
- `C++11`부터 `delete` 키워드가 생겨서 복사, 대입 연산자를 아예 삭제할 수 있다.

- `C++11` 이전에는 이런 방법들을 사용했다. 

  1. 복사, 대입 연산자를 `private` 영역에 선언하고 정의 안하기 (링커 에러 발생)

  2. `Uncopyable`과 같은 이름의 Base 클래스를 만들고, `private`영역에 대입, 복사 연산자를 정의한다.

     그리고 이 클래스를 상속받아서 복사, 대입이 불가능한 클래스를 만들면 된다. 상속은 `private`로 받아도 된다.
...
- 힙에 생기지 않는 자원은 스마트 포인터로 처리하기에 맞지 않다. 

<br>

### 항목 15 자원 관리 클래스에서 관리되는 자원은 외부에서 접근할 수 있도록 하자

- RAII  클래스에서 실제 지원을 접근할 수 있는 연산자들이 있다.
- RAII의 목적은 데이터 은닉이 아니다. 자원 해제가 실수 없이 이루어지는 것이 목적이다.
